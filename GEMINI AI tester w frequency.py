def analyze_watermark(text):
    ...
    for name, char in zero_width_chars.items():
        ...
        if count > 0:
            ...
            # Additional check to print 'Found' if any zero-width character is present
            print('Found') 
            break  # Exit loop if any zero-width character is found

    if not found:
        print('No zero-width characters found.')
        print('Not Found')  # Print 'Not Found' if no zero-width character is found

# Get input from the user
watermarked_text = input("Put the text that you want to test: ")

# Analyze the text for watermarks
